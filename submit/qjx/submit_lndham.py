...
for T in Ts:
    for delta in deltas:
        for seed_id in range(0, seed_num):
            ...
            print(f"T: {T}")
            print(f"delta: {delta}")
            print(f"lndham_seed: {lndham_seed}")
            ...
            local_path = f"/main_{sys_id}_{task_id}_{prop_id}/T_{lndham_T:0.4f}"
            ...
            local_path += f"/run_{ex_deep}_{rk_ns}_{num_tp_periods}_{num_obs_periods}" + \
                          f"/N_{lndham_N}_alpha_{lndham_alpha:0.4f}" + \
                          f'/seed_{lndham_seed}'
            ...
            for ss in range(start_seed, finish_seed, step_seed):
                ...
                fn_suffix = f"setup({sys_id}_{task_id}_{prop_id})_rnd({ss}_{mns})_N({lndham_N})_rnd({lndham_seed})_alpha({lndham_alpha:0.4f})_T({lndham_T:0.4f})"
                if task_id == 7:
