...
int main(int argc, char ** argv)
{
	...
	else
	{
		...
		FILE * diag_f = fopen("diag_nt.txt", "w");
		...
		for (int itr = 0; itr < model->conf.N_T; itr++)
		{
			...
			calcRho(model);
			for (int ii = 0; ii < model->Rho->N; ii++)
			{
				int ss = model->Rho->RowIndex[ii];
				int ff = model->Rho->RowIndex[ii + 1];
				bool f = true;
				for (int jj = ss; (jj < ff) && f; jj++)
				{
					if (model->Rho->Col[jj] == ii){
						fprintf(diag_f, "%1.10lf ", model->Rho->Value[jj]);
						f = false;
					}
				}
				if (f)
