...
void FloqManySpinsNewDelBehaviour::init_sizes(AllData* ad) const
{
	RunParam* rp = ad->rp;
	ConfigParam* cp = ad->cp;
	MainData* md = ad->md;
...
#pragma omp parallel
	...
	int n_spins = int(cp->params.find("flqnsp_n")->second);

	md->sys_size = std::round(std::pow(2, n_spins));
	md->num_diss = n_spins;
	md->num_ham_qj = 1;
	md->T = 2.0 * PI / double(cp->params.find("flqnsp_freq")->second);
}

void FloqSpinPhNewDelBehaviour::init_sizes(AllData* ad) const
{
	RunParam* rp = ad->rp;
	ConfigParam* cp = ad->cp;
	MainData* md = ad->md;
...
#pragma omp parallel
	...
	int n_ph = int(cp->params.find("flq_sp_ph_n_ph")->second);

	md->sys_size = 2 * (n_ph + 1);
	md->num_diss = 1;
	md->num_ham_qj = 1;
	md->T = 2.0 * PI / double(cp->params.find("flq_sp_ph_freq")->second);
}
...
void FloqManySpinsNewDelBehaviour::init_hamiltonians(AllData* ad) const
{
	RunParam* rp = ad->rp;
	ConfigParam* cp = ad->cp;
	MainData* md = ad->md;
	...
	double Delta = double(cp->params.find("flqnsp_Delta")->second);
	double J = double(cp->params.find("flqnsp_J")->second);
	...
	std::vector<Eigen::MatrixXcd> s_sigmas_x;
	std::vector<Eigen::MatrixXcd> s_sigmas_y;
	std::vector<Eigen::MatrixXcd> s_sigmas_z;
	for (int s_id = 0; s_id < n_spins; s_id++)
	{
		Eigen::MatrixXcd s_sigma_x(2, 2);
		Eigen::MatrixXcd s_sigma_y(2, 2);
		Eigen::MatrixXcd s_sigma_z(2, 2);
		if (s_id == 0)
		{
			s_sigma_x = sigma_x;
			s_sigma_y = sigma_y;
		...
		for (int it_id = 1; it_id < n_spins; it_id++)
		{
			if (it_id == s_id)
			{
				s_sigma_x = Eigen::kroneckerProduct(s_sigma_x, sigma_x).eval();
				s_sigma_y = Eigen::kroneckerProduct(s_sigma_y, sigma_y).eval();
				s_sigma_z = Eigen::kroneckerProduct(s_sigma_z, sigma_z).eval();
			}
			else
			{
				s_sigma_x = Eigen::kroneckerProduct(s_sigma_x, sigma_0).eval();
				s_sigma_y = Eigen::kroneckerProduct(s_sigma_y, sigma_0).eval();
				s_sigma_z = Eigen::kroneckerProduct(s_sigma_z, sigma_0).eval();
			}
		}

		s_sigmas_x.push_back(s_sigma_x);
		s_sigmas_y.push_back(s_sigma_y);
		s_sigmas_z.push_back(s_sigma_z);
	}

	Eigen::MatrixXcd H = Eigen::MatrixXcd::Zero(md->sys_size, md->sys_size);
	Eigen::MatrixXcd H_drv = Eigen::MatrixXcd::Zero(md->sys_size, md->sys_size);
	...
	{
		Eigen::MatrixXcd interaction = Eigen::MatrixXcd::Identity(md->sys_size, md->sys_size);
		for (int s_id = 0; s_id < n_spins; s_id++)
		{
			H = H + s_sigmas_z[s_id];
			interaction = interaction * s_sigmas_y[s_id];
		}
		H *= 0.5 * Delta;
		H += J * interaction;
...
void FloqSpinPhNewDelBehaviour::init_hamiltonians(AllData* ad) const
{
	RunParam* rp = ad->rp;
	ConfigParam* cp = ad->cp;
	MainData* md = ad->md;
	...
	double Delta = double(cp->params.find("flq_sp_ph_Delta")->second);
	double Omega = double(cp->params.find("flq_sp_ph_Omega")->second);
	...
	Eigen::MatrixXcd sigma_p(2, 2);
	sigma_p(0, 0) = std::complex<double>(0.0, 0.0);
	sigma_p(0, 1) = std::complex<double>(1.0, 0.0);
	sigma_p(1, 0) = std::complex<double>(0.0, 0.0);
	sigma_p(1, 1) = std::complex<double>(0.0, 0.0);
	//std::cout << sigma_p.format(CleanFmt) << sep;

	Eigen::MatrixXcd a_std = Eigen::MatrixXcd::Zero(n_ph + 1, n_ph + 1);
	for (int ph_id = 0; ph_id < n_ph; ph_id++)
	{
		a_std(ph_id, ph_id + 1) = std::sqrt(static_cast<double>(ph_id + 1));
	}
	Eigen::MatrixXcd a_dag = a_std.adjoint().eval();

	Eigen::MatrixXcd ph_eye = Eigen::MatrixXcd::Identity(n_ph + 1, n_ph + 1);

	Eigen::MatrixXcd sigma_x_full = Eigen::kroneckerProduct(ph_eye, sigma_x).eval();
	Eigen::MatrixXcd sigma_y_full = Eigen::kroneckerProduct(ph_eye, sigma_y).eval();
	Eigen::MatrixXcd sigma_z_full = Eigen::kroneckerProduct(ph_eye, sigma_z).eval();

	Eigen::MatrixXcd a_dag_sigma_minus = Eigen::kroneckerProduct(a_dag, sigma_0) * Eigen::kroneckerProduct(ph_eye, sigma_m);
	Eigen::MatrixXcd sigma_plus_a_std = Eigen::kroneckerProduct(ph_eye, sigma_p) * Eigen::kroneckerProduct(a_std, sigma_0);
...
void FloqManySpinsNewDelBehaviour::init_dissipators(AllData* ad) const
{
	ConfigParam* cp = ad->cp;
	MainData* md = ad->md;
	...
	std::vector<Eigen::MatrixXcd> disses;
	for (int s_id = 0; s_id < n_spins; s_id++)
	{
		Eigen::MatrixXcd s_sigma_m(2, 2);
		if (s_id == 0)
		{
			s_sigma_m = sigma_m;
		}
		...
		for (int it_id = 1; it_id < n_spins; it_id++)
		{
			if (it_id == s_id)
			{
				s_sigma_m = Eigen::kroneckerProduct(s_sigma_m, sigma_m).eval();
			}
			else
			{
				s_sigma_m = Eigen::kroneckerProduct(s_sigma_m, sigma_0).eval();
			}
		}

		disses.push_back(s_sigma_m);
		//std::cout << s_sigma_m.format(CleanFmt) << sep;
	}
	...
	for (int st_id_1 = 0; st_id_1 < md->sys_size; st_id_1++)
	{
		for (int st_id_2 = 0; st_id_2 < md->sys_size; st_id_2++)
		{
			...
			for (int s_id = 0; s_id < n_spins; s_id++)
			{
				md->dissipators[s_id][index].real = disses[s_id](st_id_1, st_id_2).real();
				md->dissipators[s_id][index].imag = disses[s_id](st_id_1, st_id_2).imag();
			}
		}
	}
}

void FloqSpinPhNewDelBehaviour::init_dissipators(AllData* ad) const
{
	ConfigParam* cp = ad->cp;
	MainData* md = ad->md;
	...
	Eigen::MatrixXcd diss = Eigen::kroneckerProduct(a_std, sigma_0).eval();
	...
	for (int st_id_1 = 0; st_id_1 < md->sys_size; st_id_1++)
	{
		for (int st_id_2 = 0; st_id_2 < md->sys_size; st_id_2++)
		{
			...
			md->dissipators[0][index].real = diss(st_id_1, st_id_2).real();
			md->dissipators[0][index].imag = diss(st_id_1, st_id_2).imag();
		}
	}
}
...
void FloqManySpinsNewDelBehaviour::init_hamiltonians_qj(AllData* ad) const
{
	ConfigParam* cp = ad->cp;
	MainData* md = ad->md;

	double gamma = double(cp->params.find("flqnsp_gamma")->second);
...
void FloqSpinPhNewDelBehaviour::init_hamiltonians_qj(AllData* ad) const
{
	ConfigParam* cp = ad->cp;
	MainData* md = ad->md;

	double gamma = double(cp->params.find("flq_sp_ph_gamma")->second);
...
void FloqManySpinsNewDelBehaviour::free_hamiltonians(AllData* ad) const
{
	MainData* md = ad->md;
...
void FloqSpinPhNewDelBehaviour::free_hamiltonians(AllData* ad) const
{
	MainData* md = ad->md;
...
void FloqManySpinsNewDelBehaviour::free_dissipators(AllData* ad) const
{
	MainData* md = ad->md;
...
void FloqSpinPhNewDelBehaviour::free_dissipators(AllData* ad) const
{
	MainData* md = ad->md;
...
void FloqManySpinsNewDelBehaviour::free_hamiltonians_qj(AllData* ad) const
{
	MainData* md = ad->md;
...
void FloqSpinPhNewDelBehaviour::free_hamiltonians_qj(AllData* ad) const
{
	MainData* md = ad->md;
