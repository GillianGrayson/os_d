...
crsMatrix * TensorToCrs(Tensor_Coordinates &t_ijk, int s, int f, int N)
{
   ...
   for(i = 0; i < f - s; i++)
   {
     val.re = t_ijk.data[s + i].re;
     val.im = t_ijk.data[s + i].im;
     mat->Value[i] = val;
...
//  Tensor_Coordinates *f_ijk = new Tensor_Coordinates;
//  Tensor_Coordinates *d_ijk = new Tensor_Coordinates;
