...
curr_evals_diff = diff(curr_evals);

curr_evals_diff = curr_evals_diff / mean(curr_evals_diff);

for eval_id = 1 : size(curr_evals, 1) - 1
    
    curr_eval_diff = curr_evals_diff(eval_id);
    
    if ((curr_eval_diff >= bin_begin) && (curr_eval_diff <= bin_end))
        bin_id = floor((log10(curr_eval_diff) - log10(bin_begin)) * num_bin / (log10(bin_end) - log10(bin_begin) + eps)) + 1;
