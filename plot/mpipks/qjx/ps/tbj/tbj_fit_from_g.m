...
alphas = zeros(g_num, 1);
decades = zeros(g_num, 1);
...
for g_id = 1:g_num
    ...
    if ~isnan(R2)
        alphas(g_id) = abs(alpha);
        decades(g_id) = log10(x_max) - log10(x_min);
    else
        alphas(g_id) = 0;
        decades(g_id) = 0;
    end
...
suffix_save = sprintf("diss(%d)_s(%d)_nps(%d)_diss(%d_%0.4f)_drv(%0.4f_%0.4f_%0.4f)_prm(%0.4f_%0.4f_var)", ...
    diss, ...
    ps_num_spins, ...
    ps_num_photons_states, ...
    diss_type, ...
...
hLine = plot(gs, alphas, 'LineWidth', 2);
hold all;
...
savefig(sprintf('%s/tbj_alpha_from_g_%s.fig', home_figures_path, suffix_save));

h=gcf;
set(h,'PaperOrientation','landscape');
set(h,'PaperUnits','normalized');
set(h,'PaperPosition', [0 0 1 1]);
print(gcf, '-dpdf', sprintf('%s/tbj_alpha_from_g_%s.pdf', home_figures_path, suffix_save));

fig = figure;

hLine = plot(gs, decades, 'LineWidth', 2);
hold all;
hLine = plot([gs(1) gs(end)], [0 0], 'k', 'LineStyle','--');
set(gca, 'FontSize', 20);
xlabel('$g$', 'Interpreter', 'latex');
...
savefig(sprintf('%s/tbj_decades_from_g_%s.fig', home_figures_path, suffix_save));

h=gcf;
set(h,'PaperOrientation','landscape');
set(h,'PaperUnits','normalized');
set(h,'PaperPosition', [0 0 1 1]);
print(gcf, '-dpdf', sprintf('%s/tbj_decades_from_g_%s.pdf', home_figures_path, suffix_save));
