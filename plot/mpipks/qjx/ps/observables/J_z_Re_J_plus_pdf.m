...
for run_id = 1:num_runs
    ...
    data_x = importdata(path);
	
	path = sprintf('%s/spec_3_evo_%s.txt', path_to_folder, suffix);
	data_y = importdata(path);
    
    curr_re = data_x(:, 1);
    curr_im = data_y(:, 1);
    for tr_id = 1:num_trajectories-1
        curr_re = vertcat(curr_re, data_x(:, 1 + 2*tr_id));
        curr_im = vertcat(curr_im, data_y(:, 1 + 2*tr_id));
    end
...
print(gcf, '-dpdf', sprintf('%s/J_z_Re(J_plus)_pdf_%s.pdf', home_figures_path, suffix_save));
savefig(gcf, sprintf('%s/J_z_Re(J_plus)_pdf_%s.fig', home_figures_path, suffix_save));
close(gcf)