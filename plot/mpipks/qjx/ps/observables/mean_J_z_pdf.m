...
for run_id = 1:num_runs
    ...
    for tr_id = 1:num_trajectories-1
        curr_re = vertcat(curr_re, data_x(:, 1 + tr_id));
...
print(gcf, '-dpdf', sprintf('%s/mean_J_z_pdf_%s.pdf', home_figures_path, suffix_save));
savefig(gcf, sprintf('%s/mean_J_z_pdf_%s.fig', home_figures_path, suffix_save));
close(gcf)