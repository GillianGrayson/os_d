...
for run_id = 1:num_runs
	...
	data_1 = importdata(path);
    
    curr_re = data_1(:, 1);
    curr_im = data(:, 2);
    for tr_id = 1:num_trajectories-1
        curr_re = vertcat(curr_re, data_1(:, 1 + 2*tr_id));
...
print(gcf, '-dpdf', sprintf('%s/eta_pdf_%s.pdf', home_figures_path, suffix_save));
savefig(gcf, sprintf('%s/eta_pdf_%s.fig', home_figures_path, suffix_save));
close(gcf)