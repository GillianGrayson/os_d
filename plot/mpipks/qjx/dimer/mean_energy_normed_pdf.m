...
x_min = min(mean_evo(:)) - 1e-4;
x_max = max(mean_evo(:)) + 1e-4;
mean_evo = (mean_evo - x_min) / (x_max - x_min) * 2 - 1;
mean_x = mean(mean_evo(:));
mean_evo = mean_evo - mean_x;
mean_x = mean(mean_evo(:))
x_min_tmp = min(mean_evo(:));
x_max_tmp = max(mean_evo(:));
x_min = -max(abs(x_min_tmp), abs(x_max_tmp)) - 1e-4;
x_max = max(abs(x_min_tmp), abs(x_max_tmp)) + 1e-4;
mean_evo = (mean_evo - x_min) / (x_max - x_min) * 2 - 1;
x_min = -1;
x_max = 1;
...
y_min = min(energy_evo(:)) - 1e-4;
y_max = max(energy_evo(:)) + 1e-4;
energy_evo = (energy_evo - y_min) / (y_max - y_min) * 2 - 1;
mean_y = mean(energy_evo(:));
energy_evo = energy_evo - mean_y;
mean_y = mean(energy_evo(:))
y_min_tmp = min(energy_evo(:))
y_max_tmp = max(energy_evo(:))
y_min = -max(abs(y_min_tmp), abs(y_max_tmp)) - 1e-4;
y_max = max(abs(y_min_tmp), abs(y_max_tmp)) + 1e-4;
energy_evo = (energy_evo - y_min) / (y_max - y_min) * 2 - 1;
y_min = -1;
y_max = 1;
...
savefig(sprintf('%s/mean_energy_normed_pdf_%s.fig', home_figures_path, suffix_save));

h=gcf;
set(h,'PaperOrientation','landscape');
set(h,'PaperUnits','normalized');
set(h,'PaperPosition', [0 0 1 1]);
print(gcf, '-dpdf', sprintf('%s/mean_energy_normed_pdf_%s.pdf', home_figures_path, suffix_save));
