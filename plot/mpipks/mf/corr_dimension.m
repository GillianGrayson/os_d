clear all;

home_figures_path = '/home/yusipov/Work/os_d/figures';

data_path = '/data/biophys/yusipov/os_d';
prefix = 'mf_results';

data_path = sprintf('%s/%s', data_path, prefix);

task = 5;
npps = 10000;
np = 10;

for U = 0.6 : 0.1 : 0.6

	seed_start = 1;
	seed_num = 1;
	mt = 0;
	num_steps = 10000;
	
	E = 1.0;
	A = 1.5 ;
	omega = 1.0;
	phase = 0.0;
	gamma = 0.1;
	J = 1.0;

	eps_start = 1.0e-8;
	eps_per_decade = 10;
	eps_num = 81;

	fig = figure;
	   
	for m = 1:4
		
		epss = zeros(eps_num, 1);
		cis = zeros(eps_num, 1);
		
		for eps_id = 1:eps_num
		
			U = U
			m = m
			eps_id = eps_id
			
			eps = eps_start * 10.0^((eps_id-1)/eps_per_decade);
			
			epss(eps_id) = eps;
			
			for seed = 1:seed_num
				
				path_to_folder = sprintf('%s/task_%d/np_%d/nppp_%d/mt_%d/omega_%0.4f/phase_%0.4f/g_%0.4f/J_%0.4f/E_%0.4f/A_%0.4f/U_%0.4f/eps_%0.10f/m_%d/seed_%d', ...
					data_path, ...
					task, ...
					np, ...
					npps, ...
					mt, ...
					omega, ...
					phase, ...
					gamma, ...
					J, ...
					E, ...
					A, ...
					U, ...
					eps, ...
					m, ...
					seed);
				
				fn_suffix = sprintf('mt(%d)_omega(%0.4f)_phase(%0.4f)_g(%0.4f)_J(%0.4f)_E(%0.4f)_A(%0.4f)_U(%0.4f)_seed(%d).txt', ...
						mt, ...
						omega, ...
						phase, ...
						gamma, ...
						J, ...
						E, ...
						A, ...
						U, ...
						seed);
				
				if (task == 5)
					fn_suffix = sprintf('t(%d)_eps(%0.10f)_m(%d)_mt(%d)_omega(%0.4f)_phase(%0.4f)_g(%0.4f)_J(%0.4f)_E(%0.4f)_A(%0.4f)_U(%0.4f)_seed(%d).txt', ...
						task, ...
						eps, ...
						m, ...
						mt, ...
						omega, ...
						phase, ...
						gamma, ...
						J, ...
						E, ...
						A, ...
						U, ...
						seed);
				end
				
				fn = sprintf('%s/ci_%s', path_to_folder, fn_suffix)
				data = importdata(fn);
				
				cis(eps_id) = cis(eps_id) + data;
			end
			
			cis(eps_id) = cis(eps_id) / seed_num;
			
		end
		
		hLine = plot(log10(epss), log10(cis), 'LineWidth', 2);
		legend(hLine, sprintf('%d', m));
		
		set(gca, 'FontSize', 30);
		xlabel('$\log_{10}(\epsilon)$', 'Interpreter', 'latex');
		
		set(gca, 'FontSize', 30);
		ylabel('$\log_{10}(C)$', 'Interpreter', 'latex');
		
		hold all;
		
	end

	fn_suffix = sprintf('task(%d)_np(%d)_npps(%d)_mt(%d)_omega(%0.4f)_phase(%0.4f)_g(%0.4f)_J(%0.4f)_E(%0.4f)_A(%0.4f)_U(%0.4f)_seed_num(%d)', ...
		task, ...
		np, ...
		npps, ...
		mt, ...
		omega, ...
		phase, ...
		gamma, ...
		J, ...
		E, ...
		A, ...
		U, ...
		seed_num);

	savefig(sprintf('%s/mf_cd_%s.fig', home_figures_path, fn_suffix));

end
