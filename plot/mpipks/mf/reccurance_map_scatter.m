...
for seed = 1:seed_num
	...
	x_n = coordinate(1 : end-1);
	y_n = coordinate(2 : end);
	
	x_n_global = vertcat(x_n_global, x_n);
	y_n_global = vertcat(y_n_global, y_n);
end

fig = figure;
hLine = scatter(x_n_global, y_n_global);
set(gca, 'FontSize', 30);
...
savefig(sprintf('%s/mf_rec_map_scatter_%s.fig', home_figures_path, fn_suffix));
