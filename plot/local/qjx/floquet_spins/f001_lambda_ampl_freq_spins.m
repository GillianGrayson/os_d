...
path_figures = 'D:/YandexDisk/Work/os_d/drafts/floquet/many_spins/lambdas';
...
lpn_type = -1;
lpn_delta_s = log10(1.0e-2);
lpn_delta_f_high = log10(1.0e-2);
lpn_delta_f_low = log10(1.0e-2);
...
for freq_id = 1:freq_num
    ...
    for ampl_id = 1:ampl_num
        ...
        suffix = sprintf('setup(%d_%d_%d)_rnd(%d_%d)_n(%d)_ampl(%0.4f)_freq(%0.4f)_D(%0.4f)_J(%0.4f)_gamma(%0.4f)_lpn(%d_%0.4f_%0.4f_%0.4f)', ...
            sys_id, ...
            task_id, ...
            prop_id, ...
            seed, ...
