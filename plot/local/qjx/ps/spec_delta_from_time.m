...
spec_evo_base = complex(spec_evo_data(:, 1), spec_evo_data(:, 2));

spec_evo_var = complex(spec_evo_data(:, 2 * tr_id + 1), spec_evo_data(:, 2 * tr_id + 2));

spec_evo_diff = zeros(size(spec_evo_base, 1), 1);
for i = 1 : size(spec_evo_base, 1)
    %spec_evo_diff(i) = sqrt( (real(spec_evo_base(i)) - real(spec_evo_var(i))).^2 + (imag(spec_evo_base(i)) - imag(spec_evo_var(i))).^2 );
    spec_evo_diff(i) = abs(spec_evo_base(i) - spec_evo_var(i));
end
