...
num_trs = size(mean_evo_data, 2);
mean_evo_data = mean_evo_data / size_sys;

sigma_evo = zeros(num_dumps, 1);

for dump_id = 1:num_dumps
    means_curr = mean_evo_data(dump_id, :);
    sigma_evo(dump_id) = std(means_curr);
end

fig = figure;
hLine = plot(dump_periods, sigma_evo);
